...
xlsx_path = str(cwd + "/" + project_name + "/" + today + "/" + "list.xlsx")
print(xlsx_path)
wb = openpyxl.load_workbook(xlsx_path)
ws = wb.active

for r in range(len(ws['A'])):
    try:
        search_word = ws['A'][r].value
        print(search_word)
        wsr = wb.create_sheet(search_word)
        wsr.title = search_word
        # print("검색어를 입력해주세요")
        ...
        continue
save_file = str(cwd + "/" + project_name + "/" + today + "/" + time + '.xlsx')
wb.save(save_file)
os.system("open -a 'Microsoft Excel.app' '%s'" %save_file)